...
class FPN(nn.Module):
    ...
    def __init__(self, in_channels_list, out_channels_list=None, activation=nn.ReLU):
        super().__init__()
        ...
        self.lateral = nn.ModuleList([ConvBlock(in_c, out_c, kernel_size=1, activation=activation)\
                for in_c, out_c in zip(in_channels_list, out_channels_list)])

        self.merge = nn.ModuleList([ConvBlock(out_c, out_c, kernel_size=3, activation=activation)\
    ...
    def forward(self, x):
        ...
        for i in range(1, len(y)):
            ...
            _h, _w = _y_up.shape[-2], _y_up.shape[-1]
            _y_merged = self.merge[i-1](_y_low[...,:_h,:_w] + _y_up)
            y[i] = _y_merged
        return y
