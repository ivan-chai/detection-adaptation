...
def iou(boxes_a, boxes_b):
    boxes_a = boxes_a[:, None, :].float()
    boxes_b = boxes_b[None, :, :].float()
...
class BboxConverter:
    ...
    def make_bboxes(self, deltas, sizes, pivots=None):
        if pivots is None:
            assert deltas.ndim in [3,4]
            pivots = self.make_pivots_from_shape(*deltas.shape[-3:-1])
    ...
    def make_pivots_from_indices(self, id_h, id_w):
        y_coord = id_h*self.stride + self.offsets[1]
        x_coord = id_w*self.stride + self.offsets[0]
        pivots = torch.stack([x_coord, y_coord], dim=1)
        return pivots

    def make_pivots_from_shape(self, height, width):
        y_coord = torch.arange(height)*self.stride + self.offsets[1]
        x_coord = torch.arange(width)*self.stride + self.offsets[0]
        pivots = torch.stack([
